"""Archived Tier 2 baseline: portable rule-based primitive concepts."""
...
def make_baseline_primitive_concepts() -> List[Concept]:
    """Construct archived Tier 2 rule-based concepts."""
    ...
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in FIRST_PERSON for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in SECOND_PERSON for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(
                texts, lambda t: any(tok in NEGATION or tok.endswith("n't") for tok in tokenize(t))
            ),
        )
    )
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in MODALS for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
    ...
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in NEGATIVE_WORDS for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in POSITIVE_WORDS for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
            ...
            fn=lambda texts: _binary_from_texts(texts, lambda t: any(tok in VIOLENCE_WORDS for tok in tokenize(t))),
        )
    )
    concepts.append(
        Concept(
